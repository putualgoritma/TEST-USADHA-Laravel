...
class TopupsApiController extends Controller
{
    ...
    public function transfer(Request $request)
    {
        ...
        if ($points_balance >= $total) {
            ...
            $code = acc_code_generate($last_code, 8, 3);
            //check if env transfer acc is on
            $status_transfer='pending';
            $status_mutation_transfer='onhold';
            if(env('ADMIN_ACC_TRSF')==0){
                $status_transfer='approved';
                $status_mutation_transfer='onhand';
            }
            //set topup
            $data = array_merge($request->all(), ['code' => $code, 'total' => $total, 'type' => 'transfer', 'status' => $status_transfer, 'ledgers_id' => 0, 'customers_id' => $customers_id, 'payment_type' => 'point', 'memo' => $memo]);
            $topup = Topup::create($data);
            //set topup points

            $topup->points()->attach($points_id, ['amount' => $total, 'type' => 'C', 'status' => $status_mutation_transfer, 'memo' => $memo, 'customers_id' => $customers_id]);
            $topup->points()->attach($points_id, ['amount' => $total, 'type' => 'D', 'status' => $status_mutation_transfer, 'memo' => $memo, 'customers_id' => $request->to]);
            ...
            return response()->json([
                'success' => true,
                'message' => 'Transfer Sedang Diproses, Menunggu Konfirmasi Admin.',
            ]);
        } else {
            return response()->json([
                'success' => false,
